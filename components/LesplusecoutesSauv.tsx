...
const Lesplusecoutes: React.FC = () => {
  ...
  // Initialisation de l'AudioContext et de l'analyseur audio
  useEffect(() => {
  ...
  const handlePlayOrPause = (audioRef: React.RefObject<HTMLAudioElement>, setIsPlaying: React.Dispatch<React.SetStateAction<boolean>>) => {
    ...
    <Flex>
      <Wrap spacing={4} align="center">
        {songs.map(song => {
          ...
          const [isStopped, setIsStopped] = useState(false); // Nouvel état pour la musique arrêtée
          ...
          useEffect(() => {
            ...
            if (isStopped) {
              const timer = setTimeout(() => setIsStopped(false), 2000);
              return () => clearTimeout(timer);
            }
          }, [isStopped]);

          const borderColor = isPlaying ? '#853e8a' : isHovered ? 'gray' : isStopped ? '' : 'transparent';
          ...
          return (
            <WrapItem
              ...
              onClick={() => {
                handlePlayOrPause(audioRef, setIsPlaying);
                setIsStopped(false); // Réinitialiser l'état de la musique arrêtée lors du clic
              }}
              style={{
                cursor: 'pointer',
            ...
            >
              <Flex direction="column">
                ...
                <Box mt="1rem">
                  ...
                  <canvas ref={audioRef} width="200" height="50"></canvas>
                </Box>
